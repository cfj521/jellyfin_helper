...
def _fetch_tmdb_rating_map(keys: List[tuple]) -> dict:
    ...
    if not keys:
        return {}
    from web.backend.services.metadata_store import get_batch
...
def get_ratings_batch(
    ...
    from web.backend.services import metadata_store as _ms
    ...
    for it in items:
        ...
        try:
            ...
            if douban_row is None:
                try:
                    from web.backend.cache_store import get_cached as _kv_get_local
    ...
    if unmapped_douban_ids:
        try:
            from web.backend.api.discover import _kick_douban_prefetch
