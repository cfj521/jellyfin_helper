...
历史：之前还含 Jackett 搜索 + qB 下载推送，现已拆出：
  - /search* → backend/api/resourcesearch.py
  - /download* / /downloads* / /transfer-info* → backend/api/downloadpipeline.py
"""
import sys
import logging
...
from backend.cache_store import (
    ...
    if not refresh:
        ...
        if l3_row is not None:
            if metadata_store.is_stale(l3_row):
                ...
                try:
                    from backend.services.metadata_maintenance import enqueue_refresh
...
def _normalize_tmdb(items: List[dict], default_type: Optional[str] = None) -> List[dict]:
    ...
    try:
        from backend.services.metadata_maintenance import enqueue_refresh as _enq_refresh
        ...
        try:
            ...
            if detail and detail.get('summary'):
                ...
                if detail.get('imdb_id'):
                    try:
                        from backend.api.ratings import enqueue_mdblist_by_imdb
