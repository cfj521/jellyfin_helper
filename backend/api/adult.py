...
from backend.database import get_db, SessionLocal, AdultItem
...
def list_items(
    ...
    from backend.api.medialibraries import lookup_jellyfin_item, jellyfin_web_url
...
def _list_items_from_jellyfin(
    ...
    try:
        ...
        from backend.diagnostics import timed
...
def delete_item(
    ...
    if delete_in_jellyfin and settings.jellyfin_api_key and item.file_path:
        from backend.api.medialibraries import lookup_jellyfin_item, invalidate_path_index
        from common.jellyfin_client import JellyfinClient
        jf = lookup_jellyfin_item(item.file_path)
        if jf:
            try:
...
def run_adult_scrape_batch(task_id: int, item_ids: List[int], write_nfo: bool, download_cover: bool):
    ...
    from backend.task_log_capture import attach as _log_attach
                    ...
                    if existing_poster and Path(existing_poster).exists():
                        ...
                        try:
                            ...
                            if cover_path:
                                ...
                                from backend.path_translator import reverse_translate_path_with_settings as _rev_tr
...
def _to_dict(item: AdultItem, full: bool = False) -> dict:
    ...
    from backend.path_translator import translate_path_with_settings as _tr
...
def list_actresses(
    ...
    from backend.database import AdultActress
...
def adult_library_stats(
    ...
    now = datetime.utcnow()
    from backend.path_translator import translate_path_with_settings as _stat_tr
    for i in items:
        if i.code:
            recognized += 1
