...
from backend.database import get_db, MediaItem, Task
...
from backend.cache_store import get_cached as _kv_get, set_cached as _kv_set, invalidate as _kv_invalidate
...
def _build_item_dict(i: Dict, host: str) -> Dict:
    ...
    """
    from backend.api._item_health import compute_health, extract_suggested_title_year
...
def _compute_series_subtitle_coverage(series_path: str, db: Session) -> Optional[Dict]:
    ...
    if not series_path:
        return None
    try:
        from backend.database import Task as _Task
...
def get_sample_evidence(item_id: str):
    ...
    from backend.api._sample_evidence import gather_sample_evidence
...
def library_stats(
    ...
    if library_id in adult_ids:
        from backend.api.adult import adult_library_stats
...
def library_subtitle_stats(
    ...
    """
    from backend.api.subtitle import _resolve_scope, run_subtitle_scan
