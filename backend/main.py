...
class _SilentPollAccessFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        try:
            from backend.diagnostics import _SILENT_POLL_PATHS
            args = record.args
...
from backend.task_log_capture import install as _install_task_log_capture
...
async def lifespan(app: FastAPI):
    ...
    try:
        from backend.shutdown import install_signal_handlers
    ...
    from backend.api.auth import sync_users_from_config
    sync_users_from_config()

    # 处理上次未完成的孤儿任务（必须在 API 路由 import 完成、注册表填充后才调用）
    try:
        from backend.task_restart import restart_orphan_tasks
    ...
    try:
        ...
        logger.warning(f"启动 Jellyfin 库变更轮询器失败: {e}")
    ...
    try:
        import threading
        from backend.services.metadata_maintenance import daily_loop as _meta_daily_loop
    ...
    try:
        from backend.shutdown import request_shutdown
        request_shutdown()
    except Exception:
        pass
...
from backend.diagnostics import TimingMiddleware, install_pool_monitoring
...
# ============================================================
# 前端 SPA 静态托管
# 当 frontend/dist 存在时（npm run build 产物），FastAPI 直接托管前端。
