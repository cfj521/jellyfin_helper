...
│
├── Dockerfile                     # 多阶段构建：node 编译前端 + python:3.12-slim 运行时
├── docker-compose.yml             # 5 服务编排（helper + Jellyfin + Jackett + qB + Postgres）
├── .env.example                   # compose 环境变量模板
├── scripts/
│   ├── bootstrap_stack.py         #   一站式初始化：qb/Jackett 密码 + API Key + indexer + RSS + Jellyfin 向导
│   └── docker-entrypoint.sh       #   容器入口：chown 后 gosu 降权
│
├── backend/                       # FastAPI 后端
